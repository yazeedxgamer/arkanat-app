...
// =======================================================
/**
 * دالة للتحقق مما إذا كانت ورديتان تتعارضان في الأيام والأوقات
 * @param {object} scheduleA - الوردية الأولى (يجب أن تحتوي على days, start_time, end_time)
 * @param {object} scheduleB - الوردية الثانية (يجب أن تحتوي على days, start_time, end_time)
 * @returns {boolean} - true إذا كان هناك تعارض، وإلا false
 */
function doShiftsConflict(scheduleA, scheduleB) {
    if (!scheduleA || !scheduleB || !scheduleA.days || !scheduleB.days) return false;

    // 1. التحقق من وجود يوم مشترك
    const hasCommonDay = scheduleA.days.some(day => scheduleB.days.includes(day));
    if (!hasCommonDay) {
        return false; // لا يوجد تعارض إذا لم تكن هناك أيام عمل مشتركة
    }

    // 2. التحقق من تعارض الأوقات (مع التعامل مع الورديات الليلية)
    const startA = scheduleA.start_time;
    const endA = scheduleA.end_time;
    const startB = scheduleB.start_time;
    const endB = scheduleB.end_time;

    // دالة مساعدة لتحويل الوقت إلى دقائق من بداية اليوم
    ...
    let endAMinutes = timeToMinutes(endA);
    const startBMinutes = timeToMinutes(startB);
    let endBMinutes = timeToMinutes(endB);

    // إذا كانت الوردية ليلية، أضف 24 ساعة (1440 دقيقة) للنهاية
    if (endAMinutes < startAMinutes) endAMinutes += 1440;
    if (endBMinutes < startBMinutes) endBMinutes += 1440;

    // الشرط الأساسي للتعارض: بداية أحدهما قبل نهاية الآخر، ونهاية أحدهما بعد بداية الآخر
    const conflict = startAMinutes < endBMinutes && endAMinutes > startBMinutes;
    const conflictOvernight = (startAMinutes + 1440) < endBMinutes && (endAMinutes + 1440) > startBMinutes;
    const conflictOvernightB = startAMinutes < (endBMinutes + 1440) && endAMinutes > (startBMinutes + 1440);


    return conflict || conflictOvernight || conflictOvernightB;
}


// =======================================================
// ===     بداية الإضافة: دالة تنسيق الوقت الموحدة      ===
// =======================================================
...
async function fetchAndDisplayOpenShifts() {
    ...
    data.forEach(shift => {
        ...
        const displayTime = createShiftTimeLabel(shift);
...
document.getElementById('coverage-application-form')?.addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (isEmployee) {
            // منطق الموظف الحالي (مع التحقق من التعارض)
            ...
            // --- بداية الإضافة: جلب بيانات الموظف وجدوله ---
            const { data: user, error: userError } = await supabaseClient
                .from('users')
                .select('id, name, job_vacancies!users_vacancy_id_fkey(schedule_details)')
                .eq('id_number', employeeIdNumber)
                .single();
            ...
            const { data: coverageShift, error: shiftError } = await supabaseClient
                ...
                .eq('id', shiftId)
                .single();
            if (shiftError || !coverageShift) throw new Error('لم يتم العثور على وردية التغطية.');
            
            const coverageShiftDate = new Date(coverageShift.created_at);
            const coverageShiftDay = coverageShiftDate.toLocaleString('en-US', { weekday: 'short' });

            const coverageSchedule = {
                days: [coverageShiftDay],
                start_time: coverageShift.start_time,
                end_time: coverageShift.end_time
            };

            // التحقق من التعارض مع الجدول الأساسي للموظف
            const primarySchedule = user.job_vacancies?.schedule_details?.[0];
            if (primarySchedule) {
                const periods = primarySchedule.periods || [primarySchedule];
                for (const period of periods) {
                    const periodSchedule = { ...period, days: primarySchedule.days };
                    if (doShiftsConflict(coverageSchedule, periodSchedule)) {
                        throw new Error('لا يمكنك التقديم على هذه التغطية لأنها تتعارض مع وقت ورديتك الأساسية.');
                    }
                }
            }
            // --- نهاية الإضافة ---
            ...
            applicationData = {
                ...
                full_name: user.name, 
                id_number: employeeIdNumber,
                status: 'pending_ops'
            };
        } else {
            // منطق المتقدم الخارجي (يبقى كما هو)
